...
class Session(models.Model):
    ...
    seats = fields.Integer(string="Number of seats", default=50, )
    ...
    def _check_instructor_not_in_attendees(self):
        for r in self:
            if r.instructor_id in r.attendee_id :
                raise exceptions.ValidationError(_("A session's instructor can't be an attendee"))
            if r.attendee_id in r.instructor_id :
                raise exceptions.ValidationError(_("A session's attendee can't be an instructor"))